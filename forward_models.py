...
def b_fwd_1d (r, R):
    ...
    """
    ratio = r/R
    b = np. sqrt (np. square ( ratio ) + 1) - np. abs ( ratio )
    return b

def fwd_model_1d (arr , x, z, R):
    """
...
def fwd_model_fake2d (arr , x1 , x2 , z):
    """
    Apply fwd model to space - time array arr ( must be on grid of spatial
    locs ); predicts at same time points as input .
    ( Note : want dense input within integral bounds to get good results !)
    Output can be requested on non - grid .
    ...
    for t in range (nt):
        ...
        for i in range (nz):
            ...
            wt = 1/np.sqrt(deltax1**2+deltax2**2)
            toint = wt * arr_tmp # (nx1 , nx2 )
            res [i, t] = scipy . integrate . trapz ( scipy . integrate . trapz (toint ,
                x=np. squeeze (x1), axis =0) , x=np. squeeze (x2), axis =0)
    return res

def b_fwd_fake2d ( delta1 , delta2 , R, eps ):
    """
    ...
    """
    wt = 1/np.sqrt(delta1**2+delta2**2)
    return wt
...
def fwd_model_2d (arr , x1 , x2 , z, R, eps ):
    """
    Apply fwd model to space - time array arr ( must be on grid of spatial
    locs ); predicts at same time points as input .
    ( Note : want dense input within integral bounds to get good results !)
...
def b_fwd_fake2d ( delta1 , delta2):
    """
    Computes b weight function from forward model for parameters R, epsilon (to handle singularity ).
    : param delta1 : differences between spatial coords dimension 1 ( vector )
    : param delta2 : differences between spatial coords dimension 2 ( vector )
